    ...
    public static void main(String[] args) {
        ...
        Map<String, String> topRatedMoviesByGenre = movies.stream()
                .collect(Collectors.groupingBy(
                        Movie::getGenre, // 장르별로 그룹화
                        Collectors.collectingAndThen(
                                Collectors.maxBy(Comparator.comparingDouble(Movie::getRating)), // 최고 평점을 가진 영화 찾기
                                optionalMovie -> optionalMovie.map(Movie::getTitle).orElse("영화 없음") // 영화의 제목 추출
                        )
                ));
